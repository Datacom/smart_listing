...
module SmartListing
  class Base
    ...
    def setup params, cookies
      ...
      if @options[:array]
        if @sort && @sort.present? # when array we sort only by first attribute
        ...
        #
        @collection = @collection.order(sort_keys.collect{|s| "#{s[1]} #{@sort[s[0]]}" if @sort[s[0]]}.compact) if @sort && @sort.present?
